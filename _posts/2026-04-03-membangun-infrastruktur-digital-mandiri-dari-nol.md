...
pin: true
image:
  path: /assets/img/posts/2026-04-03-membangun-infrastruktur-digital-mandiri-dari-nol/thumbnail.webp
...
Observability memang menjawab pertanyaan "apakah layanan berjalan?", tetapi tidak menjawab "apakah ada yang mencoba menyusup?". Ketika infrastruktur Anda mulai menyimpan data sensitif — kredensial, dokumen pribadi, arsip komunikasi — lapisan berikutnya yang wajib dibangun adalah pemantauan keamanan.

Untuk itu, Wazuh hadir sebagai platform SIEM/XDR open-source yang menggabungkan File Integrity Monitoring (FIM), deteksi rootkit, Security Configuration Assessment (SCA), dan analisis log host dalam satu ekosistem. Agar data keamanan yang dihasilkan Wazuh dapat dikorelasikan dengan metrik operasional yang sudah dikumpulkan Prometheus, integrasikan Wazuh ke dalam Grafana sebagai data source terpadu.

> Mulai dari integrasi data source melalui [Integrasi Wazuh ke Grafana — Unified SOC Dashboard dengan Plugin Data Source](https://docs.ricalnet.my.id/posts/integrasi-wazuh-ke-grafana-unified-soc-dashboard-dengan-plugin-data-source/), lalu lengkapi dengan konfigurasi agent yang bersih dari noise melalui panduan yang sama. Dengan begitu, alert keamanan (SSH brute force, modifikasi file kritis, CVE) dan metrik operasional (CPU, memori, status container) tersedia dalam satu antarmuka Grafana.
{: .prompt-tip}
...
Untuk visibilitas kejadian tersebut, integrasikan Wazuh sebagai lapisan deteksi. Jika Wazuh belum terpasang, mulai dari [Integrasi Wazuh ke Grafana — Unified SOC Dashboard dengan Plugin Data Source](https://docs.ricalnet.my.id/posts/integrasi-wazuh-ke-grafana-unified-soc-dashboard-dengan-plugin-data-source/); setelah itu konfigurasikan agent host agar tidak menghasilkan alert palsu dengan mengikuti panduan yang sama.
...
Untuk membangun lapisan ketiga, mulai dari [Integrasi Wazuh ke Grafana — Unified SOC Dashboard dengan Plugin Data Source](https://docs.ricalnet.my.id/posts/integrasi-wazuh-ke-grafana-unified-soc-dashboard-dengan-plugin-data-source/), lalu lanjutkan dengan panduan yang sama agar agent yang Anda pasang benar-benar menghasilkan sinyal yang dapat ditindaklanjuti, bukan sekadar noise. Dengan integrasi ini, seluruh sinyal keamanan dan operasional tersedia dalam satu antarmuka Grafana.
...
- [Konfigurasi Wazuh Agent untuk Keamanan Host — FIM, Rootcheck, dan SCA](https://docs.ricalnet.my.id/posts/konfiguras-wazuh-agent-untuk-keamanan-host-fim-rootcheck-dan-sca)

- [Integrasi Wazuh ke Grafana — Unified SOC Dashboard dengan Plugin Data Source](https://docs.ricalnet.my.id/posts/integrasi-wazuh-ke-grafana-unified-soc-dashboard-dengan-plugin-data-source/)
