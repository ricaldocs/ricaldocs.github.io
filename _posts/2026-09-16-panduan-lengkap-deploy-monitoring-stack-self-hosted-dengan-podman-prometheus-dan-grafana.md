...
```
  dashdot                        0.0652 cores
  uptime-kuma                    0.0127 cores
  immich_machine_learning        0.0121 cores
  nextcloud_app                  0.0099 cores
  linkstack                      0.0087 cores
```
...
- Ntfy terintegrasi untuk notifikasi push

Yang perlu diingat:
- Grafana di port 3003 (bukan 3000)
- Ntfy di dua network (`monitoring_network` + `ntfy_network`)
- Password di `.env`
- Semua service auto-start via systemd user service
